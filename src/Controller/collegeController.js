...
const getCollegeDEtails=async(req,res)=>{
      ...
      if(!data){
        return res.status(404).send({status:false,message:"collegename name not found"})
      }
      let saveddata=await internModel.find({collegeId:data._id}).select({_id:1,name:1,email:1,mobile:1});
      if(!saveddata){
